...
function AvgStep_and_D = DS_Diagonal_AvgStep_and_D(m)

    % For a fixed initial and final points 
    xi=1; yi=1; xf=25; yf=25;
    
    % a = x-axis length, b = y-axis length, m = number of static obstacles
    a=50; b=50;
    ...
    % workspace. (Not a hard-and-fast rule though)
    
    % The obstacles are placed diagonally only.
    O_x = 2:m+1 ; O_y = 2:m+1; % m <= a-4, or m <= b-4
    
    % Calculating D
    
    D = 0;
    for i=1:m
        D = D + sqrt(sum(([O_x(i),O_y(i)] - [xi,yi]) .^ 2)) + sqrt(sum(([O_x(i),O_y(i)] - [xf,yf]) .^ 2));
    end
