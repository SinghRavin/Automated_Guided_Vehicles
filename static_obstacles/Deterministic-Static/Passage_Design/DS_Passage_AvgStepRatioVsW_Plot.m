...
for i = 1:length(workspace_dim)
    for j = 1:W
        Avg_Step(i,j) = DS_Passage_AvgStep(workspace_dim(i),workspace_dim(i),j);
        j
    end
end
...
for i = 1:length(workspace_dim)
    for j = 1:W
        ratio(i,j) = Avg_Step(i,j)/(sqrt(2*((workspace_dim(i)-1)^2))*j);
    end
end
...
%title('initial=(1,1), final=(a-1,b-1), walk=300, cutoff=1e6, Obstacles region = rectangular passage of width W')
...
legend({'a*b = 50*50','a*b = 75*75','a*b = 100*100','a*b = 125*125'},'Location','northeast');
title('Obstacles region = Rectangular passage of width W')
