...
function AvgStep_and_Dnew = RS_AvgStep_and_Dnew(m)
    ...
    % Calulating Dnew i.e., Scatter Within
    
    Means = RS_Obstacles_Mean(O_x, O_y);
    mu_x = Means(1);
    mu_y = Means(2);
    ...
    for i=1:m
        for j=1:m
            Dnew = Dnew + sum((mat(i,j).position - [mu_x,mu_y]) .^ 2);
        end
    end
